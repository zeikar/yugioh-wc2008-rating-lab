...
export function TournamentsPage() {
  ...
  const drafts = isAdmin ? listDrafts().sort((a, b) => b.number - a.number) : []

  return (
    <>
      <PageTitle aside={isAdmin && <button className="btn btn-primary" onClick={start}>+ New tournament</button>}>Tournaments</PageTitle>
          ...
          <ul className="panel divide-y divide-rule">
            {drafts.map((d) => (
              <li key={d.id} className="flex items-center justify-between px-4 py-2 text-sm">
                ...
                <Link to={`/tournaments/${d.id}`} className="font-medium text-accent hover:underline">
                  Continue
                </Link>
              </li>
            ))}
      ...
      <div className="panel overflow-x-auto">
        {tournaments.length === 0 ? (
          <Empty>No tournaments recorded yet.{isAdmin ? ' Start one with “New tournament” while the game is running.' : ''}</Empty>
        ) : (
            ...
            <tbody>
              {tournaments.map((t) => {
                ...
                return (
                  <tr key={t.id}>
                    <td>
                      <Link to={`/tournaments/${t.id}`} className="font-semibold text-accent hover:underline">
                        #{t.number}
                      </Link>
                    </td>
